...
const RechargePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (paymentMethod === 'zalopay') {
      ...
      setLoading(true);
      try {
        // Gọi backend tạo đơn hàng nạp tiền trước
        const res = await request.post("recharge/zalopaySubmit", { customMoney: formData.amount });
        if (res.code !== 1) {
            toast.error(res.msg || "Lỗi tạo đơn nạp tiền");
            setLoading(false);
            return;
        }
        const orderNo = res.data.payment.order_no;

        // Gọi API thanh toán của Zalo Mini App
        Payment.createOrder({
          desc: `Nạp tiền ${orderNo}`,
          item: JSON.stringify([{ order_no: orderNo, order_type: 100 }]),
