...
(function(){
    ...
    UM.registerWidget( widgetName,{
        ...
        tpl: "<link rel=\"stylesheet\" type=\"text/css\" href=\"<%=video_url%>video.css\" />" +
            ...
            "<tr>" +
            "   <td>" +
            "       <label for=\"eduiVideoUrl\" class=\"edui-video-url\"><%=lang_video_url%></label>" +
            "   </td>" +
            "   <td>" +
            "       <input id=\"eduiVideoUrl\" type=\"text\" placeholder=\"请输入视频url（必填）\">" +
            "   </td>" +
            "</tr>" +
            "<tr>" +
            "   <td>" +
            "       <label for=\"eduiVideoPoster\" class=\"edui-video-poster\">视频封面</label>" +
            "   </td>" +
            "   <td><input id=\"eduiVideoPoster\" type=\"text\" placeholder=\"请输入封面图url（非必填）\"></td>" +
            "</tr>" +
            // "<tr>" +
            // "   <td>" +
            // "       <label for=\"eduiVideoPoster\" class=\"edui-video-poster\">自动播放</label>" +
            // "   </td>" +
            // "   <td>" +
            // "       <label>" +
            // "           <input class='eduiVideoAutoPlay' name=\"auto_play\" type=\"radio\" checked>" +
            // "           <span> 否 </span>" +
            // "       <label>" +
            // "       <label>" +
            // "           <input class='eduiVideoAutoPlay' name=\"auto_play\" type=\"radio\">" +
            // "           <span> 是 </span>" +
            // "       <label>" +
            // "   </td>" +
            // "</tr>" +
            ...
            "<tr>" +
            "   <td><label for=\"eduiVideoHeight\"><%=lang_videoH%></label></td>" +
            "   <td><input class=\"edui-video-txt\" id=\"eduiVideoHeight\" value=\"190\" type=\"text\"/></td>" +
            "</tr>" +
            "</table>" +
            "</fieldset>" +
            "<fieldset>" +
        ...
        createPreviewVideo: function(url){
            ...
            $("#eduiVideoPreview", me.$widget)[0].innerHTML = '<video src="' + url + '" controls></video>';
        ...
        insertSingle: function(){
            ...
            var me = this,
                ...
                poster = $('#eduiVideoPoster', me.$widget)[0].value,
                align = this.findFocus("eduiVideoFloat","name");

            if(!url) return false;
            if ( !me.checkNum( [ height] ) ) return false;
