    ...
    private function getOauthInfo(array $partyData): ?array
{
    ...
    // 🔵 ADD LINE LOGIN HERE
    // =========================
    if ($partyData['oauth'] === 'LINE') {

        // ví dụ: frontend gửi access_token hoặc code
        $accessToken = $partyData['access_token'] ?? '';

        // gọi LINE API verify user
        ...
        $lineUser = json_decode($result, true);

        if (empty($lineUser['sub'])) {
            throwError("LINE login failed");
        }

        return [
            'oauth_id' => $lineUser['sub'],
            'unionid' => $lineUser['sub'] ?? null
        ];
    }
