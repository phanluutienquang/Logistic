...
namespace app\web\service\order;
...
use app\web\model\UserCoupon as UserCouponModel;
use app\web\model\dealer\Order as DealerOrderModel;

use app\web\service\User as UserService;
use app\web\service\Payment as PaymentService;
use app\web\service\coupon\GoodsDeduct as GoodsDeductService;
use app\web\service\web\GoodsDeduct as webDeductService;
use app\web\service\order\source\checkout\Factory as CheckoutFactory;
...
class Checkout
{
     ...
     * @return bool
     */
    private function setOrderweb()
    {
        // 设置默认的商品积分抵扣信息
        $this->setDefaultGoodsweb();
        // 积分设置
        $setting = SettingModel::getItem('web');
        // 条件：后台开启下单使用积分抵扣
        if (!$setting['is_shopping_discount'] || !$this->checkoutRule['is_use_web']) {
            return false;
        }
        // 条件：订单金额满足[?]元
        ...
        // 计算订单商品最多可抵扣的积分数量
        $this->setOrderGoodsMaxwebNum();
        // 订单最多可抵扣的积分总数量
        $maxwebNumCount = helper::getArrayColumnSum($this->goodsList, 'max_web_num');
        // 实际可抵扣的积分数量
        $actualwebNum = min($maxwebNumCount, $this->user['web']);
        if ($actualwebNum < 1) {
            return false;
        }
        // 计算订单商品实际抵扣的积分数量和金额
        $GoodsDeduct = new webDeductService($this->goodsList);
        $GoodsDeduct->setGoodsweb($maxwebNumCount, $actualwebNum);
        // 积分抵扣总金额
        $orderwebMoney = helper::getArrayColumnSum($this->goodsList, 'web_money');
        $this->orderData['web_money'] = helper::number2($orderwebMoney);
        // 积分抵扣总数量
        $this->orderData['web_num'] = $actualwebNum;
        // 允许积分抵扣
        $this->orderData['is_allow_web'] = true;
        return true;
    }
     ...
     * @return bool
     */
    private function setOrderGoodsMaxwebNum()
    {
        // 积分设置
        $setting = SettingModel::getItem('web');
        foreach ($this->goodsList as &$goods) {
            // 商品不允许积分抵扣
            if (!$goods['is_web_discount']) continue;
            // 积分抵扣比例
            ...
            $maxwebMoney = helper::bcmul($totalPayPrice, $deductionRatio);
            // 最多可抵扣的积分数量
            $goods['max_web_num'] = helper::bcdiv($maxwebMoney, $setting['discount']['discount_ratio'], 0);
        }
        return true;
    }
     ...
     * @return bool
     */
    private function setDefaultGoodsweb()
    {
        foreach ($this->goodsList as &$goods) {
            // 最多可抵扣的积分数量
            $goods['max_web_num'] = 0;
    ...
    private function getOrderData()
    {
        ...
        return [
            ...
            'is_allow_web' => false,
            // 是否使用积分抵扣
            'is_use_web' => $this->param['is_use_web'],
            // 积分抵扣金额
            'web_money' => 0.00,
            // 赠送的积分数量
            'web_bonus' => 0,
    ...
    private function getSetting()
    {
        ...
        $deliveryType = SettingModel::getItem('store')['delivery_type'];
        // 积分设置
        $webSetting = SettingModel::getItem('web');
        // 订阅消息
        $orderSubMsgList = [];
        ...
        return [
            ...
            'web_name' => $webSetting['web_name'],      // 积分名称
            'web_describe' => $webSetting['describe'],     // 积分说明
     ...
     * @return bool
     */
    private function setOrderwebBonus()
    {
        // 初始化商品积分赠送数量
        foreach ($this->goodsList as &$goods) {
            $goods['web_bonus'] = 0;
        ...
        foreach ($this->goodsList as &$goods) {
            ...
            // 计算抵扣积分数量
            $goods['web_bonus'] = !$goods['is_web_gift'] ? 0 : helper::bcmul($goods['total_pay_price'], $ratio, 0);
        }
        //  订单积分赠送数量
        $this->orderData['web_bonus'] = helper::getArrayColumnSum($this->goodsList, 'web_bonus');
        return true;
    }
    ...
    private function setOrderGoodsPayPrice()
    {
        ...
        foreach ($this->goodsList as &$goods) {
            ...
            // 减去积分抵扣金额
            if ($this->orderData['is_allow_web'] && $this->orderData['is_use_web']) {
                $value = helper::bcsub($value, $goods['web_money']);
            }
    ...
    private function createOrderEvent($order)
    {
        ...
        // 积分抵扣情况下扣除用户积分
        if ($order['is_allow_web'] && $order['is_use_web'] && $order['web_num'] > 0) {
            $describe = "用户消费：{$this->model['order_no']}";
            $this->user->setIncweb(-$order['web_num'], $describe);
        }
        // 获取订单详情
     ...
     * @return bool
     */
    private function isExistwebDeduction($order)
    {
        return $order['is_allow_web'] && $order['is_use_web'];
    }
    ...
    {
        // 当前订单是否存在和使用积分抵扣
        $isExistwebDeduction = $this->isExistwebDeduction($order);
        // 订单数据
        $data = [
            ...
            'web_money' => $isExistwebDeduction ? $order['web_money'] : 0.00,
            'web_num' => $isExistwebDeduction ? $order['web_num'] : 0,
    ...
    private function saveOrderGoods($order)
    {
        ...
        foreach ($order['goods_list'] as $goods) {
            ...
            $item = [
                ...
                'web_money' => $isExistwebDeduction ? $goods['web_money'] : 0.00,
                'web_num' => $isExistwebDeduction ? $goods['web_num'] : 0,
                'web_bonus' => $goods['web_bonus'],
