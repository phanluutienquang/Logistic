...
class Shop extends ShopModel
{
     ...
     * 
     */
    public function getQueryList($where=null,$limit=10)
    { 
      // 获取数量
      $limit != false && $this->limit($limit);
      // 获取门店列表数据
      $data = $this->setWhere($where)->where('is_delete', '=', '0')
          ->where('status', '=', '1')
          ->order(['sort' => 'asc', 'create_time' => 'desc'])
          ->select();
      // 根据距离排序
    ...
    public function setWhere($where){
        if (isset($where['shop_name']) && $where['shop_name']){
           return $this->where('shop_name','like','%'.$where['shop_name'].'%');
        } 
        return $this;
    }  
    ...
    public function getDetails($id){
       return $this->field('shop_id,shop_name,province_id,city_id,region_id,address,phone,post')->find($id);
    }
