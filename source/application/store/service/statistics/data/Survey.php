...
class Survey extends BasicsService
{
    ...
    {
        return number_format((new InpackModel)->getPayOrderTotal($startDate, $endDate));
    }
    ...
    {
        return helper::number2((new InpackModel)->getOrderTotalPrice($startDate, $endDate));
    }
    ...
    private function getGoodsTotal($startDate = null, $endDate = null)
    {
        $model = new LineModel;
        if (!is_null($startDate) && !is_null($endDate)) {
            $model->where('created_time', '>=', strtotime($startDate))
                ->where('created_time', '<', strtotime($endDate) + 86400);
        }
        $value = $model->count();
        return number_format($value);
    }
