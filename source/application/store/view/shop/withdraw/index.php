        ...
        <div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
            <div class="widget am-cf">
                <div class="widget-head am-cf">
                    <div class="widget-title am-cf">加盟商提现申请</div>
                </div>
                                ...
                                <div class="am fr">
                                    ...
                                    <div class="am-form-group am-fl">
                                        <select name="pay_type"
                                            ...
                                            <option value="40" <?= $request->get('pay_type') == '40' ? 'selected' : '' ?>>
                                                微信好友转账
                                            </option>
                                        </select>
                                    </div>
                    ...
                    <div class="am-scrollable-horizontal am-u-sm-12 am-padding-bottom-lg">
                        <table width="100%" class="am-table am-table-compact am-table-striped
                         tpl-table-black am-text-nowrap">
                            ...
                            <tbody>
                            <?php $payType = [10=>'微信零钱提现',20=>'支付宝',30=>'银行卡',40=>'微信转账']; ?>    
                            <?php if (!$list->isEmpty()): foreach ($list as $item): ?>
                                <tr>
                                    <td class="am-text-middle"><?= $item['shop_id'] ?></td>
                                    <td class="am-text-middle">
                                        <a href="<?= $item['shop']['logo']['file_path'] ?>" title="点击查看大图" target="_blank">
                                            <img src="<?= $item['shop']['logo']['file_path'] ?>"
                                                 width="50" height="50" alt="">
                                        </a>
                                    </td>
                                    <td class="am-text-middle">
                                        <p><span><?= $item['shop']['shop_name'] ?></span></p>
                                    </td>
                                    <td class="am-text-middle">
                                        <?php if (!empty($item['shop']['linkman']) || !empty($item['shop']['phone'])): ?>
                                            <p><?= $item['shop']['linkman'] ?: '--' ?></p>
                                            <p><?= $item['shop']['phone'] ?: '--' ?></p>
                                        <?php else: ?>
                                            <p>--</p>
                                        <?php endif; ?>
                                    ...
                                    <td class="am-text-middle">
                                        <p><span><?= $payType[$item['pay_type']['value']]; ?></span></p>
                                    </td>
                                    <td class="am-text-middle">
                                        <?php if ($item['pay_type']['value'] == 20) : ?>
                                            <p><span>户主：<?= $item['alipay_name'] ?></span></p>
                                            <p><span>账号：<?= $item['alipay_account'] ?></span></p>
                                        <?php elseif ($item['pay_type']['value'] == 30) : ?>
                                            <p><span>银行：<?= $item['bank_name'] ?></span></p>
                                            <p><span>户主：<?= $item['bank_account'] ?></span></p>
                                            <p><span>卡号：<?= $item['bank_card'] ?></span></p>
                                        <?php elseif ($item['pay_type']['value'] == 40) : ?>
                                            <p><span>微信号：<?= $item['weixinhao'] ?></span></p>
                                        <?php else : ?>
                                            <p><span>--</span></p>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td class="am-text-middle">
                                        <div class="tpl-table-black-operation">
                                            <?php if (in_array($item['apply_status'], [10, 20])) : ?>
                                                <?php if (checkPrivilege('shop.withdraw/submit')): ?>
                                                    <a class="j-audit" data-id="<?= $item['id'] ?>"
                                                       href="javascript:void(0);">
                                                        <i class="iconfont icon-shenhe"></i> 审核
                                                    </a>
                                                <?php endif; ?>
                                                <?php if ($item['apply_status'] == 20 && in_array($item['pay_type']['value'], [20, 30, 40])) : ?>
                                                    <?php if (checkPrivilege('shop.withdraw/money')): ?>
                                                ...
                                                <?php if ($item['apply_status'] == 20 && $item['pay_type']['value'] == 10) : ?>
                                                    <?php if (checkPrivilege('shop.withdraw/wechat_pay')): ?>
...
<script>
    $(function () {
         ...
         * 余额打款
         */
        $('.j-yue').click(function () {
            var id = $(this).data('id');
            var url = "<?= url('shop.withdraw/payToMoney') ?>";
            layer.confirm('确定已打款吗？', {title: '友情提示'}, function (index) {
                $.post(url, {id: id}, function (result) {
                    result.code === 1 ? $.show_success(result.msg, result.url)
                        : $.show_error(result.msg);
