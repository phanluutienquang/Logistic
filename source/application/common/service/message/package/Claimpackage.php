 ...
 */
class Claimpackage extends Basics
{
    /**
     * 参数列表
     * @var array
     ...
     */
    private $pageUrl = [
        OrderTypeEnum::MASTER => 'pages/index/index',
        OrderTypeEnum::SHARING => 'pages/index/index',
    ];
    ...
    private function onSendWxTplMsg()
    {
        ...
        // 获取订阅消息配置
        $template = SettingModel::getItem('tplMsg',$this->param['wxapp_id'])['claimpackage'];
        ...
        if ($storesetting['client']['mode']==10) {
            ...
            'data' => [
                ...
                $template['keywords'][2] => ['value' => $orderInfo['member_id']],
                $template['keywords'][3] => ['value' => $orderInfo['calim_time']],
                $template['keywords'][4] => ['value' => $orderInfo['weight']],
            ]
        ]);
        }else{
